...
class SI7021(object):
   ...
   def get_humidity(self):
       ...
       if msgs == None:
           return FAILURE, "Nothing Read", None
       else:
   ...
   #    print "\nGet Temp - no hold split"
       ...
       if msgs == None:
           ...
           value = bytesToWord(msgs[0].data[0], msgs[0].data[1])
           return SUCCESS, None, self.calc_temp(value)
...
def test():
   ...
   """
    ...
    print("\nTest Temp - previous")
    status, comment, temp = si.get_tempC_prior()
    if status != SUCCESS:
        print("Error getting Temp", comment)
    else:        
        print('Temp C: %.2f C' % temp)
